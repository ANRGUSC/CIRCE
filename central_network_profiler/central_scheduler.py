...
for cur_node, row in df_rel.iterrows():
    ...
    temp_schedule = pd.DataFrame(columns=['Destination','Node','Region','Password'])
    temp_schedule=temp_schedule.append({'Destination':cur_node,'Node':dict_rel.get(cur_node)[0],'Region':row['Region'],'Password':row['Password']},ignore_index=True)
    temp_schedule=temp_schedule.append(temp[['Destination','Node','Region','Password']],ignore_index=False)
...
sched.add_job(do_update_quadratic,'interval', id='update',minutes=5,replace_existing=True)
sched.start()
while True:
    time.sleep(10)
#sched.shutdown()