...
class HEFT:
    def __init__(self, filename):
        ...
        """
        self.num_task, self.num_processor, comp_cost, self.rate, self.data,self.quaratic_profile = init(filename)
        ...
        for p in self.processors:
            for duration in p.time_line:
                if duration.task_num != -1:
                    output.write("task"+str(duration.task_num+1) + " node"+ str(p.number+1))
                    output.write('\n') 
