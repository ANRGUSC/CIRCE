...
for task in task_order:
    ...
    k = timeit.Timer("module.main()", globals = globals())
    mytime = min(k.repeat(1,1))/1
    filename = module.main()
    output_data = file_size(module.main())
    myfile.write('{0:<16s} {1:<15.10f} {2:s} \n'.format(task, mytime, str(output_data)))
...
sftp=ssh.open_sftp()
file_path = os.path.join('apac_scheduler', 'profiler_files', 'profiler_'+nodename+'.txt')
